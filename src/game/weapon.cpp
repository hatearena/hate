...
guninfo guns[NUMGUNS] = {
    ...
    {S_LIGHTGUN, 50, 15, 0, 0, 0, "lightgun"},
...
bool intersect(dynent *d, vec &from,
               vec &to) // if lineseg hits entity bounding box
{
  vec v = to, w = d->o, *p;
  vsub(v, from);
  vsub(w, from);
...
void hit(int target, int damage, dynent *d, dynent *at) {
  if ((m_teammode || m_infected) && at && d != at && at->team[0] && d->team[0] && !strcmp(at->team, d->team))
    return;
  if (d == player1)
            ...
            bool local) // create visual effect from a shot
{
  ...
  case GUN_SG: {
    loopi(SGRAYS) {
      particle_splash(0, 8, 300, sg[i]);
      particle_splash(1, 2, 400, sg[i]);
    };
    break;
  };
