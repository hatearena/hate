...
void spawnmonster() // spawn a random monster according to freq distribution in
                    // DMSP
{
...
void monsterclear() // called after map start to spawn all monsters to initial
                    // state
{
...
bool los(float lx, float ly, float lz, float bx, float by, float bz,
    ...
    dynent *m) // main AI thinking routine, called every frame for every monster
{
  ...
  if (m->blocked) // special case: if we run into scenery
  {
    m->blocked = false;
    if (!rnd(20000 /
             monstertypes[m->mtype].speed)) // try to jump over obstackle (rare)
    {
      m->jumpnext = true;
    ...
    {
      m->targetyaw +=
          180 + rnd(180); // patented "random walk" AI pathfinding (tm) ;)
  ...
  case M_SLEEP: // state classic sp monster start in, wait for visual contact
  {
    ...
    if (disttoenemy < 8 // the better the angle to the player, the further the
                        // monster can see/hear
        || (disttoenemy < 16 && angle < 135) ||
        (disttoenemy < 32 && angle < 90) || (disttoenemy < 64 && angle < 45) ||
        angle < 10) {
  ...
  case M_HOME: // monster has visual contact, heads straight for player and may
               // want to shoot at any time
    m->targetyaw = enemyyaw;
    if (m->trigger < lastmillis) {
      vec target;
      if (!enemylos(m, target)) // no visual contact anymore, let monster get as
                                // close as possible then search for player
      {
        transition(m, M_HOME, 1, 800, 500);
      } else // the closer the monster is the more likely he wants to shoot
      {
        if (!rnd((int)disttoenemy / 3 + 1) &&
            m->enemy->state == CS_ALIVE) // get ready to fire
        {
...
void monsterrender() {
  loopv(monsters) renderclient(
      monsters[i], false, monstertypes[monsters[i]->mtype].mdlname,
