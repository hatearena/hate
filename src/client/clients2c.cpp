...
void localservertoclient(uchar *buf,
                         int len) // processes any updates from the server
{
  ...
  while (p < end)
    ...
    case SV_POS: // position of another client
    {
      ...
      int f = getint(p);
      if (cn >= BOT_CLIENT_BASE && d->maxspeed == 48.0f) {
        entinmap(d);
        d->maxspeed = 49.0f;
      }
    ...
    case SV_DIED: {
      int actor = getint(p);
      if (m_infected && d->team[0] && strcmp(d->team, "INFD") != 0) {
        dynent *killer = NULL;
        if (actor == clientnum)
          killer = player1;
        else
          killer = getclient(actor);
        if (killer && killer->team[0] && !strcmp(killer->team, "INFD")) {
