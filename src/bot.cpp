...
static void botaction(dynent *m)
{
    ...
    if(player1->state == CS_ALIVE)
    {
        ...
        if(dist < bestdist) { bestdist = dist; enemy = player1; }
    }
    ...
    loopv(players)
    {
        ...
        if(dist < bestdist) { bestdist = dist; enemy = o; }
    }

    loopv(bots)
    {
        dynent *o = bots[i];
        if(o == m || o->state != CS_ALIVE) continue;
        vdist(dist, v, m->o, o->o);
        if(dist < bestdist) { bestdist = dist; enemy = o; }
    }

    if(!enemy) { m->move = 0; m->strafe = 0; return; }
    m->enemy = enemy;
...
{
    if(d != m && d->state == CS_ALIVE) m->enemy = d;
...
void addbotcmd()
{
    if(isdedicated) { conoutf("bots not supported on dedicated servers"); return; }
    if(multiplayer()) { conoutf("only the server admin can spawn bots"); return; }
    if(gamemode < 0) { conoutf("bots only work in multiplayer modes"); return; }
