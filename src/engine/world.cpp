...
void settag(int tag, int type) // set all cubes with "tag" to space, if tag is 0
                               // then reset ALL tagged cubes according to type
{
...
void remip(block &b, int level) {
  ...
  for (int x = s.x; x < s.x + s.xs; x += 2)
    for (int y = s.y; y < s.y + s.ys; y += 2) {
      ...
      o[3] = SWS(w, x, y + 1, ws);
      sqr *r =
          SWS(v, x / 2, y / 2, vs); // the target cube in the higher mip level
      *r = *o[0];
      uchar nums[MAXTYPE];
      loopi(MAXTYPE) nums[i] = 0;
      loopj(4) nums[o[j]->type]++;
      r->type = SEMISOLID; // cube contains both solid and space, treated
      ...
      if (SOLID(r)) {
        ...
        loopi(3) {
          if (o[i]->type != o[3]->type || o[i]->floor != o[3]->floor ||
              o[i]->ceil != o[3]->ceil || o[i]->ftex != o[3]->ftex ||
              o[i]->ctex != o[3]->ctex ||
              abs(o[i + 1]->r - o[0]->r) >
        ...
        if (r->type == CHF || r->type == FHF) // can make a perfect mip out of a
