              ...
              persistent_entity &light) // done in realtime, needs to be fast
{
  ...
  int stepl = fast_f2nat(l / (float)steps); // incorrect: light will fade
                                            // quicker if near edge of the world
  ...
  if (hasoverbright) {
    ...
    {
      int dimness =
          rnd((255 - (light.attr2 + light.attr3 + light.attr4) / 3) / 16 + 1);
      x += stepx * dimness;
      y += stepy * dimness;
...
void postlightarea(block &a) // median filter, smooths out random noise in light
...
void dodynlight(vec &vold, vec &v, int reach, int strength, dynent *owner) {
  ...
  persistent_entity l = {(int)v.x, (int)v.y, (int)v.z, reach,
                         LIGHT,    strength, 0,        0};
  calclightsource(l);
  postlightarea(b);
};
