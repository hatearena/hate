...
void render_seg_new(float vx, float vy, float vh, int mip, int x, int y, int xs,
  ...
  int lx = vxx - lodleft; // these mark the rect inside the current rest that we
                          // want to render using a lower mip level
  int ly = vyy - lodtop;
  int rx = vxx + lodright;
  int ry = vyy + lodbot;
  ...
  for (int ox = x; ox < xs; ox++)
         ...
         oy++) // first collect occlusion information for this block
    {
      SWS(w, ox, oy, sz)->occluded =
          isoccluded(vx, vy, (float)(ox << mip), (float)(oy << mip), fsize);
    };
...
#define LOOPD                                                                  \
  ...
  LOOPH // ceils
  {
    ...
    render_seg_new(vx, vy, vh, mip - 1, xx * 2, start * 2, xx * 2 + 2,
                   yy * 2 + 2);
    continue;
  };
  stats[mip]++;
...
if (s->type == CORNER) {
  ...
  if (SOLID(z)) {
    if (SOLID(w)) {
      render_wall(w, h2 = s, xx + 1, yy, xx, yy + 1, mip, t, v, false);
      topleft = false;
    } else if (SOLID(v)) {
      render_wall(v, h2 = s, xx, yy, xx + 1, yy + 1, mip, s, u, false);
    };
  } else if (SOLID(t)) {
    if (SOLID(w)) {
      render_wall(w, h1 = s, xx + 1, yy + 1, xx, yy, mip, u, s, false);
    } else if (SOLID(v)) {
      render_wall(v, h1 = s, xx, yy + 1, xx + 1, yy, mip, v, t, false);
      topleft = false;
    };
  } else {
    normalwall = false;
