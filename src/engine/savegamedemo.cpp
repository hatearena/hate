...
void loadstate(char *fn) {
  ...
  if (gzgetc(f) != islittleendian)
    goto out; // not supporting save->load accross incompatible architectures
  ...
  nextmode = gzgeti();
  changemap(mapname); // continue below once map has been loaded and client &
                      // server have updated
  return;
out:
...
void loadgamerest() {
  ...
  loopv(monsters) {
    ...
    monsters[i]->lastaction = monsters[i]->trigger =
        lastmillis + 500; // also lazy, but no real noticable effect on game
...
void demoplaybackstep() {
  while (demoplayback && lastmillis >= playbacktime) {
    ...
    int extras;
    if (extras = gzget()) // read additional client side state not present in
                          // normal network stream
    {
  ...
  if (demoplayback) {
    ...
    loopvrev(playerhistory) if (playerhistory[i]->lastupdate <
                                itime) // find 2 positions in history that
                                       // surround interpolation time point
    {
      ...
      *player1 = *b;
      if (a != b) // interpolate pos & angles
      {
        ...
        // if(a==z || b==c) printf("* %d\n", lastmillis);
        float bf =
            (itime - a->lastupdate) / (float)(b->lastupdate - a->lastupdate);
        fixwrap(a, player1);
        fixwrap(c, player1);
        fixwrap(z, player1);
        ...
        if (dist < 16) // if teleport or spawn, dont't interpolate
        {
          ...
          catmulrom(*(vec *)&z->yaw, *(vec *)&a->yaw, *(vec *)&b->yaw,
                    *(vec *)&c->yaw, bf, *(vec *)&player1->yaw);
        };
        fixplayer1range();
      };
      break;
